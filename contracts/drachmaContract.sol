...
contract Miceys is IERC20, Ownable  {
    ...
    uint256 private _totalFees = _devFee + _liquidityFee + _charityFee + _burnFee;
    ...
    uint256 private _originalSupply = 999_999_999_999_999;
    ...
    // Uniswap.
    address public routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; //Uni/Pancakeswap.
    ...
    uint256 private _charityHoldings;
    uint256 private _liquidityHoldings;

    // Stats.
    uint256 public _launchTimestamp;
    uint256 private _charityPaid = 0;
    mapping (address => uint256) private _charityPaidToHolder;
    ...
    constructor(string memory tokenName, string memory tokenTicker) {
        ...
        emit Transfer(address(0), _msgSender(), _holdings[_msgSender()]);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 fromBalance = _holdings[from];
        require(fromBalance >= amount, "ERC20: transfer amount exceeds balance");

        uint256 afterFeeAmount = amount;
        if (!_isExempt[from] || !_isExempt[to]) {
            uint256 devFee = _devFee.div(amount).mul(10);
            uint256 burnFee = _burnFee.div(amount).mul(10);
            uint256 liquidityFee = _liquidityFee.div(amount).mul(10);
            uint256 charityFee = _charityFee.div(amount).mul(10);
            uint256 totalFees = _totalFees.div(amount).mul(10);

            afterFeeAmount -= totalFees;

            // Dev fee.
            _holdings[DEV] += devFee;
            // Burn fee.
            _currentSupply -= burnFee;
            // Liquidity and charity.
            _holdings[address(this)] += (liquidityFee + charityFee);
            _charityHoldings += charityFee;
            _liquidityHoldings += liquidityFee;
        }

        unchecked {
            _holdings[from] = fromBalance - amount;
        }
        _holdings[to] += afterFeeAmount;
